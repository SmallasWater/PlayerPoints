...
public class PaySubCommand extends SubCommand {
    ...
    public PaySubCommand(PlayerPoints plugin) {
        super(plugin);
    }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if(args.length > 2){
            ...
            if(i == 0){
                sender.sendMessage("§c您的"+Point.getPointName()+"并不够哦");
                return true;
            }else if(i == -1){
                sender.sendMessage("§c未找到玩家 "+target+" 相关数据");
                return true;
            }else{
                sender.sendMessage(PlayerPoints.getInstance().getLanguage().getString("player.point.pay")
